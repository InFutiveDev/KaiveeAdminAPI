...
const auth = async (req, res, next) => {
  const { logger } = req;
  try {
    let token = req.body.token || req.query.token || req.headers['x-auth-token'] || req.headers["authorization"];
    // console.log("token-->",token)
    if (_.isUndefined(token)|| _.isEmpty(token) || token.length == 0 || token.toString() === 'null') {
      const obj = {
        res,
